...
class MenuViewTest(TestCase):
    def setUp(self) -> None:
        ...
        for i in range(3):
            MenuItem.objects.create(id=i + 1, title=f"Cheeseburger {i + 1}", price=8.5 + .5 * i, inventory=i + 1)
    ...
    def test_create(self):
        ...
        self.assertTrue(MenuItem.objects.filter(title='Tortilla', price=4.5, inventory=8, ).exists())
    ...
    def test_update_item(self):
        menuitem_id = 1
        new_menuitem_data = {'title': 'Tortilla 2', 'price': 5, 'inventory': 9}
        request = self.factory.put(reverse('menu-single-item', args=[menuitem_id]), data=new_menuitem_data, content_type='application/json')
        request.user = self.user
        response = SingleMenuItemView.as_view()(request, pk=menuitem_id)
        response.render()
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(response.content, {'id': 1, 'title': 'Tortilla 2', 'price': '5.00', 'inventory': 9})

    def test_partially_update_item(self):
        menuitem_id = 1
        new_menuitem_data = {'inventory': 99}
        request = self.factory.patch(reverse('menu-single-item', args=[menuitem_id]), data=new_menuitem_data, content_type='application/json')
        request.user = self.user
        response = SingleMenuItemView.as_view()(request, pk=menuitem_id)
        response.render()
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(response.content, {'id': 1, 'title': 'Cheeseburger 1', 'price': '8.50', 'inventory': 99})
...
class BookingViewTest(TestCase):
    ...
    def test_create(self):
        ...
        response.render()
        self.assertEqual(response.status_code, 201)
        self.assertTrue(Booking.objects.filter(name='Rogue', no_of_guests=6, bookingDate='2023-08-01T00:00:00Z', ).exists())
    ...
    def test_update_reservation(self):
        reservation_id = 1
        new_reservation = {'name': 'Rogue 2', 'no_of_guests': 7, 'bookingDate': '2023-08-02T00:00:00Z', }
        request = self.factory.put(reverse('reservation-detail', args=[reservation_id]), data=new_reservation)
        request.user = self.user
        response = BookingViewSet.as_view({'put': 'update'})(request, pk=reservation_id)
        response.render()
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(response.content, {'id': 1, 'name': 'Rogue 2', 'no_of_guests': 7, 'bookingDate': '2023-08-02T00:00:00Z'})

    def test_partially_update_reservation(self):
        reservation_id = 1
        new_reservation = {'no_of_guests': 99, }
        request = self.factory.patch(reverse('reservation-detail', args=[reservation_id]), data=new_reservation)
        request.user = self.user
        response = BookingViewSet.as_view({'patch': 'partial_update'})(request, pk=reservation_id)
        response.render()
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(response.content, {'id': 1, 'name': 'Bard 1', 'no_of_guests': 99, 'bookingDate': '2023-09-01T00:00:00Z'})
